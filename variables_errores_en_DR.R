...
# dataset[, ctrx_quarter_normalizado := ctrx_quarter]           #### Esta linea da error
...
if (infinitos_qty > 0) {
  ...
  dataset[mapply(is.infinite, dataset)] <<- NA        # Esta linea da error, se soluciona on <--
}
...
if (nans_qty > 0) {
  ...
  dataset[mapply(is.nan, dataset)] <<- 0          # Esta linea da error, se soluciona con <--
}